...
import { MapPin, DollarSign, Bot, Plus, X, Clock, ChevronRight, Globe } from "lucide-react";
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      <motion.div initial={{y:"100%"}} animate={{y:0}} exit={{y:"100%"}}
        ...
        <div className="px-4 sm:px-6 py-4 space-y-5 pb-safe">
...
export default function VacanciesPage() {
  ...
  return (
    <div>
        ...
        <div className="flex gap-1.5 flex-wrap">
          {(["open","paused","closed"] as const).map((s)=>(
            <span key={s} className={`text-[11px] font-mono px-2.5 py-1 rounded-full border font-medium ${STATUS[s]}`}>
              {MOCK.filter(v=>v.status===s).length} {STATUS_RU[s]}
            </span>
          ))}
        </div>
        <button className="flex items-center gap-1.5 bg-[hsl(var(--primary))] text-white text-xs font-semibold px-3 py-2 rounded-lg hover:opacity-90">
        ...
        {MOCK.map((v,i)=>(
          <motion.div key={v.id} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:i*.04}}
            ...
            <div className="flex items-start justify-between mb-2.5">
              ...
              <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full border font-semibold ${STATUS[v.status]}`}>{STATUS_RU[v.status]}</span>
            </div>
            ...
            <p className="text-xs text-[hsl(var(--muted-foreground))] mb-3 truncate-2 leading-relaxed">{v.description}</p>
      ...
      <AnimatePresence>{selected&&<VacancyModal v={selected} onClose={()=>setSelected(null)}/>}</AnimatePresence>
    </div>
  );
}
